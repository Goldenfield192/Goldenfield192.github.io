...
>[!NOTE]
> IR动画原理是将每个部件的默认状态记作0，READOUT定义的变换后状态记作1，并将这个值存储于对应[控制组](Groups.md)。
> 玩家操作即为改变这个动画变量。
...
IR通过识别`TL_[num]_[axis]`来标记移动。对于一个部件，其每个轴向上都最多允许有一个`TL`。
...
IR通过识别`SCALE_[num]_[axis]`来标记移动。对于一个部件，其每个轴向上都最多允许有一个`SCALE`。
...
整个部件会绕着旋转轴对象旋转`[deg]`度，但笔者不清楚是顺时针还是逆时针。具体值需自行测试。

该部件最好是一个一边长度明显大于另两边的长方体，这样更利于人类识别。
